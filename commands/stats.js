...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("stats")
        .setDescription("Returns a level distribution chart of all characters in the database")
    ...
    async execute(interaction) {
        try {
            ...
            // Get the user's choice for grouping
            ...
            if (characterData.length === 0) {
                ...
                let labels, levelCounts;

                // Single level grouping
                if (grouping === 'single') {
                    levelCounts = Array(18).fill(0); // Levels 3 to 20 (18 levels)

                    // Count occurrences of each level
                    levels.forEach(level => {
                        if (level >= 3 && level <= 20) {
                            levelCounts[level - 3] += 1;
                        }
                    });
                    ...
                    levelCounts = Array(6).fill(0); // 6 groups: 3-5, 6-8, ..., 18-20

                    // Count occurrences within each group
                    levels.forEach(level => {
                        if (level >= 3 && level <= 20) {
                            const groupIndex = Math.floor((level - 3) / 3); // Group levels by three
                            levelCounts[groupIndex] += 1;
                        }
                    });

                    // Labels for the X-axis (3-level groupings)
                ...
                const config = {
                    ...
                    options: {
                        ...
                        plugins: {
                            title: {
                                display: true,
                                text: grouping === 'single'
                                    ? 'Character Level Distribution (Single Levels)'
                                    : 'Character Level Distribution (Grouped by Levels)'
                            },
                            // Add this hook to fill the background with white
                            beforeDraw: (chart) => {
                                const ctx = chart.ctx;
                                ctx.save();
                ...
                const fileName = grouping === 'single' ? 'level-distribution-single.png' : 'level-distribution-grouped.png';
                const imagePath = path.join(__dirname, `../images/${fileName}`);
                fs.writeFileSync(imagePath, imageBuffer);

                // Send the file
                await interaction.editReply({
                    content: `The (${grouping === 'single' ? 'single level' : 'grouped'}) level distribution chart has been generated.`,
                    files: [{
